...
# fact, 10677is the first number to be shown to require over fifty iterations
...
def lychrel_numbers(limit)
  lychrels = []
  (1..limit).each { |number| lychrels << number if is_lychrel?(number) }
  lychrels.length
end

puts "Answer: #{ lychrel_numbers(10000) }"
puts "Time: #{ Benchmark.measure { lychrel_numbers(1000) } }"
